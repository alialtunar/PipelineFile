        ...
        stage('Build and Push image') {
            steps {
                script {
                    ...
                    withDockerRegistry([credentialsId: 'acaracar', url: '']) {
    ...
    steps {
        script {
            ...
            if (existingContainerId) {
                sh "sshpass -p '" + SECOND_SERVER_PASSWORD + "' ssh '" + SECOND_SERVER_USERNAME + "@" + SECOND_SERVER_IP + "' 'docker stop ${existingContainerId}'"
                echo "java-app-container durduruldu."
                
                sh "sshpass -p '" + SECOND_SERVER_PASSWORD + "' ssh '" + SECOND_SERVER_USERNAME + "@" + SECOND_SERVER_IP + "' 'docker rm ${existingContainerId}'"
